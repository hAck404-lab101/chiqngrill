      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-16 md:grid-cols-[0.9fr_1.1fr] md:py-24">
        <div>
          ...
          <div className="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.04] p-5">
                ...
                {cartItems.map((item) => (
                  <div key={item.id} className="grid gap-4 rounded-3xl border border-white/10 bg-black/20 p-4 md:grid-cols-[1fr_auto] md:items-center">
                    ...
                    <div className="flex items-center justify-between gap-3 md:justify-end">
                      <button type="button" onClick={() => handleQuantityChange(item.id, item.quantity - 1)} className="grid size-10 place-items-center rounded-full border border-white/10 font-black hover:border-gold hover:text-gold">−</button>
            ...
            <p className="mt-3 text-sm leading-6 text-cream/55">Delivery fee and final availability should be confirmed by the restaurant during WhatsApp checkout.</p>
          </div>
        </div>
        ...
        <div className="rounded-[2.5rem] border border-white/10 bg-cream p-6 text-charcoal md:p-8">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 md:grid-cols-4">
            {orderModes.map((mode) => (
              <button key={mode} onClick={() => setOrderMode(mode)} className={`rounded-full border px-4 py-3 text-sm font-black transition ${orderMode === mode ? "border-charcoal bg-charcoal text-cream" : "border-charcoal/10 bg-white text-charcoal/70 hover:border-flame hover:text-flame"}`}>{mode}</button>
            ))}
          </div>

          <div className="mt-8 grid gap-4">
            <label className="grid gap-2 text-sm font-bold">Full name<input value={name} onChange={(event) => setName(event.target.value)} className="rounded-2xl border border-charcoal/10 bg-white px-4 py-4 outline-none focus:border-flame" placeholder="Your name" /></label>
            <label className="grid gap-2 text-sm font-bold">Phone number<input value={phone} onChange={(event) => setPhone(event.target.value)} className="rounded-2xl border border-charcoal/10 bg-white px-4 py-4 outline-none focus:border-flame" placeholder="024 XXX XXXX" /></label>
            {orderMode === "Delivery" ? <label className="grid gap-2 text-sm font-bold">Delivery address<textarea value={address} onChange={(event) => setAddress(event.target.value)} className="min-h-28 rounded-2xl border border-charcoal/10 bg-white px-4 py-4 outline-none focus:border-flame" placeholder="Share your area, landmark, and exact delivery point" /></label> : null}
            <label className="grid gap-2 text-sm font-bold">Order notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} className="min-h-28 rounded-2xl border border-charcoal/10 bg-white px-4 py-4 outline-none focus:border-flame" placeholder="Extra spicy, no onions, pickup time, etc." /></label>
          </div>

          <a href={isCartEmpty ? "/menu" : orderSummary} className={`mt-7 block rounded-full px-7 py-4 text-center font-black transition ${isCartEmpty ? "bg-charcoal text-cream hover:bg-flame hover:text-charcoal" : "bg-flame text-charcoal hover:bg-gold"}`}>{isCartEmpty ? "Add Items First" : "Send Order on WhatsApp"}</a>
          <p className="mt-4 text-center text-sm text-charcoal/55">MVP checkout only. Backend order saving and Paystack payment come next.</p>
        </div>
      </section>
    </main>
  );
