...
const occasions = ["Casual visit", "Date night", "Birthday", "Friends hangout", "Business lunch"];
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-16 md:grid-cols-[0.85fr_1.15fr] md:py-24">
        <div>
          <SectionHeading
            ...
            title="Book the vibe before you arrive."
            description="Reserve a table for dine-in, birthdays, casual visits, and relaxed hangouts with good food and soothing music."
          />
          <div className="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
            ...
            <div className="mt-5 grid gap-3">
              {occasions.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-black/20 px-4 py-4 font-bold text-cream/75">{item}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <form className="rounded-[2.5rem] border border-white/10 bg-cream p-6 text-charcoal md:p-8">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-bold md:col-span-2">Special request<textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-28 rounded-2xl border border-charcoal/10 bg-white px-4 py-4 outline-none focus:border-flame" placeholder="Birthday setup, seating preference, arrival note..." /></label>
          </div>
