...
type AuditLog = { id: string; action: string; actor: string; details?: Record<string, unknown>; createdAt: string };
...
export default function AdminAuditPage() {
  ...
  async function loadLogs() {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/admin/audit-logs`, { headers: { Authorization: `Bearer ${getAdminToken()}` } });
    ...
    <AdminShell title="Audit Logs" description="Track important system actions like orders, reservations, menu edits, settings changes, and kitchen status updates.">
      ...
      <section className="rounded-[28px] bg-white p-4 shadow-[0_18px_50px_rgba(36,23,19,0.08)] ring-1 ring-black/5 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-2xl font-black">System Activity</h2><p className="mt-1 text-sm font-semibold text-[#76675d]">Latest actions are shown first.</p></div>
        ...
        <div className="mt-5 grid gap-3">{logs.map((log) => <article key={log.id} className="rounded-[24px] bg-[#fff8ef] p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black text-[#16110d]">{log.action}</p><p className="mt-1 text-sm font-bold text-[#76675d]">Actor: {log.actor || "system"}</p></div><p className="text-xs font-black text-[#9d431f]">{log.createdAt ? new Date(log.createdAt).toLocaleString() : "No date"}</p></div><p className="mt-3 rounded-2xl bg-white p-3 text-xs font-bold leading-5 text-[#76675d] ring-1 ring-black/5">{log.details ? JSON.stringify(log.details) : "No extra details"}</p></article>)}</div>
      </section>
    </AdminShell>
  );
}
