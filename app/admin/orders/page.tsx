...
export default function AdminOrdersPage() {
  ...
  return (
    <AdminShell title="Orders" description="View customer orders from the ordering app. Status editing is available through the backend and will be wired next.">
      ...
      <section className="rounded-[28px] bg-white p-5 shadow-[0_18px_50px_rgba(36,23,19,0.08)] ring-1 ring-black/5">
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="rounded-full bg-[#efe0d0] px-4 py-2 text-sm font-black text-[#9d431f]">{orders.length} total</span>
        </div>
        ...
        <div className="mt-5 grid gap-3">
          ...
          {orders.map((order) => (
            <article key={String(order.reference)} className="rounded-2xl bg-[#fff8ef] p-4">
